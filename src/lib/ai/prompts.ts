/**
 * Enhanced AI Prompts for App Generation
 * Split into planning and code generation phases for better reliability
 */

// ============================================
// PHASE 1: APP PLANNING PROMPT
// Generates structure without code
// ============================================
...
// Generates JavaScript based on the plan
// ============================================
...
// ============================================
// LEGACY: SINGLE-SHOT PROMPT (IMPROVED)
// For cases where we need everything at once
// ============================================
...
{
  "version": "1.0",
  "metadata": { "name": "Todo List", "description": "Manage tasks", "icon": "✅" },
  "inputs": [
  ...
  "code": "async function run(inputs, helpers) { const { task, priority } = inputs; if (task && task.trim()) { await helpers.db.store('todo', { task: task.trim(), priority: priority || 'medium', completed: false, createdAt: new Date().toISOString() }); } const todos = await helpers.db.getAll('todo'); const pending = todos.filter(t => !t.completed); return { message: task ? 'Task added!' : 'Your tasks', pendingCount: pending.length, pendingTasks: pending.map(t => ({ Task: t.task, Priority: t.priority })) }; }",
  "outputs": [
    { "id": "message", "type": "text", "source": "{{message}}" },
    { "id": "stats", "type": "card", "label": "Progress", "source": "{{pendingCount}} pending tasks" },
    { "id": "tasks", "type": "table", "label": "Pending", "source": "{{pendingTasks}}" }
  ]
}
...
{
  "version": "1.0",
  "metadata": { "name": "Text Summarizer", "description": "Summarize text with AI", "icon": "🤖" },
  "inputs": [
    { "id": "text", "type": "textarea", "label": "Text", "required": true, "placeholder": "Paste text here..." },
  ...
  "code": "async function run(inputs, helpers) { const { text, length } = inputs; if (!text || !text.trim()) { return { summary: 'Please enter text to summarize', stats: '' }; } const lengths = { brief: '1-2 sentences', medium: '1 paragraph', detailed: '2-3 paragraphs' }; const summary = await helpers.ai('Summarize in ' + (lengths[length] || '1 paragraph') + ': ' + text); return { summary: summary, stats: 'Original: ' + text.length + ' chars → Summary: ' + summary.length + ' chars' }; }",
  "outputs": [
    { "id": "summary", "type": "markdown", "label": "Summary", "source": "{{summary}}" },
    { "id": "stats", "type": "text", "label": "Stats", "source": "{{stats}}" }
  ]
}
